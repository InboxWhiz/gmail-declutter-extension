let declutterTabOpen = false;
let selectedSenders = {};

// Functions to build UI

async function insertDeclutterButton() {
  const button = await loadHTMLFragment(
    "content/ui/declutter_icon.html",
    "content/ui/declutter_icon.css",
    "declutter-button-style",
  );
  ...
  // Append to Gmail
  const supportIcon = Array.from(document.querySelectorAll("*")).find(
    (el) => el.getAttribute("data-tooltip") === "Support",
  );
  supportIcon.insertAdjacentElement("beforebegin", button);
}

async function insertDeclutterBody() {
  const decutterBody = await loadHTMLFragment(
    "content/ui/declutter_body.html",
    "content/ui/declutter_body.css",
    "declutter-body-style",
  );
...
async function insertModalPopups(body) {
  // Add modal popups
  const noSendersModal = await loadModalPopup(
    "content/ui/modal_popups/no_sender.html",
  );
  const deleteConfirmModal = await loadModalPopup(
    "content/ui/modal_popups/delete_confirm.html",
  );
  const deletePendingModal = await loadModalPopup(
    "content/ui/modal_popups/delete_pending.html",
  );
  const deleteSuccessModal = await loadModalPopup(
    "content/ui/modal_popups/delete_success.html",
  );
  ...
  deleteConfirmModal.querySelector(".show-emails").onclick = () =>
  ...
  body.querySelector("#unsubscribe-button").onclick = () =>
    openModal(noSendersModal, noSendersModal);
  body.querySelector("#delete-button").onclick = () =>
    openModal(deleteConfirmModal, noSendersModal);
}

async function createSenderLine(senderName, senderEmail, emailCountNum) {
  const senderLine = await loadHTMLFragment(
    "content/ui/sender_line.html",
    "content/ui/sender_line.css",
    "sender-line-style",
  );
  ...
  senderLine
    ...
    .addEventListener("change", updateCheckbox);
  ...
  function updateCheckbox() {
    ...
    console.log(
      "Total emails: " +
        Object.values(selectedSenders).reduce((a, b) => a + b, 0),
    );
  }
...
async function insertSenders(sendersList) {
  let senderLines = [];
  for (const i in sendersList) {
    let line = await createSenderLine(
      sendersList[i][1],
      sendersList[i][0],
      sendersList[i][2],
    );
  ...
  setTimeout(() => {
    ...
    declutterBodyTable.parentElement.querySelector(
      ".loading-message",
    ).style.display = "none";

    for (const line in senderLines) {
...
async function loadModalPopup(htmlUrl) {
  const modal = await loadHTMLFragment(
    htmlUrl,
    "content/ui/modal_popups/modal_popup.css",
    "modal-style",
  );
...
function openModal(confirmModal, warningModal) {
  if (Object.keys(selectedSenders).length > 0) {
    confirmModal.querySelector("#emails-num").textContent = Object.values(
      selectedSenders,
    ).reduce((a, b) => a + b, 0);
    confirmModal.querySelector("#senders-num").textContent =
      Object.keys(selectedSenders).length;
