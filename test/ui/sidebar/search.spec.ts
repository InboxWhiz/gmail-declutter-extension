...
test.describe("Search functionality", () => {
  ...
  test("should display search input", async ({ page }) => {
    ...
    await expect(searchInput).toHaveAttribute("placeholder", "Search senders...");
  });
  ...
  test("should filter senders by email address", async ({ page }) => {
    ...
    await expect(page.locator(".sender-line-real")).toContainText("alice@email.com");
  });
  ...
  test("should filter senders by name", async ({ page }) => {
    ...
    await expect(page.locator(".sender-line-real")).toHaveCount(1);
    await expect(page.locator(".sender-line-real")).toContainText("bob@email.com");
  });
  ...
  test("should filter multiple senders with partial match", async ({ page }) => {
    const searchInput = page.locator('input[aria-label="Search senders"]');
    await searchInput.fill("e@email.com");

    await page.waitForSelector(".sender-line-real", { state: "visible", timeout: 5000 });
  ...
  test("should show no results message when no matches found", async ({ page }) => {
    ...
    await expect(page.locator("#senders")).toContainText('No senders match "nonexistent@email.com"');
  });
  ...
  test("should clear search and restore all senders when clear button clicked", async ({ page }) => {
    const searchInput = page.locator('input[aria-label="Search senders"]');
    const clearButton = page.locator('button[aria-label="Clear search"]');
  ...
  test("should maintain search state when selecting/deselecting senders", async ({ page }) => {
    const searchInput = page.locator('input[aria-label="Search senders"]');
  ...
  test("should work with action buttons on filtered results", async ({ page }) => {
    const searchInput = page.locator('input[aria-label="Search senders"]');
