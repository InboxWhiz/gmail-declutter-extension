import { searchEmailSenders, deleteSenders, getAllSenders } from "../src/utils/actions";
...
describe("searchEmailSenders", () => {
    test("should log the search and send a message to the active tab if found", () => {
        ...
        (chrome.tabs.query as jest.Mock).mockImplementation((queryObj: any, callback: (tabs: any[]) => void) => {
            callback([{ id: 123 }]);
        });
